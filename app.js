...
const baseProducts = [
  ...
  { id: "sold-a", name: "初级体能药水", quality: "green", rarity: "#37a476", price: 6000, stock: 0, maxStock: 10 },
...
function renderProducts() {
  ...
  const products = [...productSets[selectedCategory]].sort((a, b) => {
    ...
    if (stateA === 2) return (qualityOrder[a.quality] ?? 99) - (qualityOrder[b.quality] ?? 99);
    const discountOrder = Number(Boolean(b.discount)) - Number(Boolean(a.discount));
    if (discountOrder !== 0) return discountOrder;
    if (stateA === 1 || (a.discount && b.discount)) return (qualityOrder[a.quality] ?? 99) - (qualityOrder[b.quality] ?? 99);
    return 0;
  });
...
function showUnlockTip(card, product) {
  ...
  const content = unlockTip.parentElement;
  const margin = 24;
  const left = Math.max(margin, Math.min(card.offsetLeft + card.offsetWidth - 40, content.clientWidth - unlockTip.offsetWidth - margin));
  const preferredTop = productGrid.offsetTop + card.offsetTop - productGrid.scrollTop + 185;
  const top = Math.max(margin, Math.min(preferredTop, content.clientHeight - unlockTip.offsetHeight - margin));
  const raised = top < preferredTop;
  unlockTip.classList.toggle("tip-raised", raised);
  const arrow = unlockTip.querySelector(".tip-arrow");
  arrow.style.top = raised
    ? `${Math.max(90, Math.min(preferredTop - top + 30, unlockTip.offsetHeight - 60))}px`
    : "30px";
  unlockTip.style.left = `${left}px`;
  unlockTip.style.top = `${top}px`;
}
